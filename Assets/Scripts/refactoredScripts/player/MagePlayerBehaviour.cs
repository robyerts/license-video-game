    ...
    public override void Hit()
    {
        ...
        switch(abilitiesTypes[attackNr])
        {
            case MageAbilityInstantiateType.DmgIntantiateOnEnemy:
                ...
                StartCoroutine(ProcessAbility(instatiatedAbility, abilitiesDmgs[attackNr], abilitiesTimeBe4Attack[attackNr], abilitiesTimeBe4Destroy[attackNr], MageAbilityType.Dmg)); // hardcoded numbers and some bellow aswell
                break;
            ...
            case MageAbilityInstantiateType.HealingInstatiateOnSelf:
                instatiatedAbility = Instantiate(abilitiesPrefabs[attackNr], abilitiesInstatiateTr[attackNr].position, abilitiesInstatiateTr[attackNr].rotation);
                StartCoroutine(ProcessAbility(instatiatedAbility, abilitiesDmgs[attackNr], abilitiesTimeBe4Attack[attackNr], abilitiesTimeBe4Destroy[attackNr], MageAbilityType.Healing));
                break;

            case MageAbilityInstantiateType.DmgAOEInstatiateOnEnemies:
                instatiatedAbility = Instantiate(abilitiesPrefabs[attackNr], abilitiesInstatiateTr[attackNr].position, abilitiesInstatiateTr[attackNr].rotation);
                StartCoroutine(ProcessAbility(instatiatedAbility, abilitiesDmgs[attackNr], abilitiesTimeBe4Attack[attackNr], abilitiesTimeBe4Destroy[attackNr], MageAbilityType.AOEDmg));
                break;
        }
    }
